...
interface TransitionLedger {
  ...
  getLineage(lineageId: string): Promise<TransitionEvent[]>;
}
```
...
actions. Carries provenance, correlationId, causationId, lineageId,
...
**Phase 1 simplification:** No standalone `LiteEnvelope` type. Existing types
(`SwarmEnvelope`, queue items) already carry their own traceability fields and
serve this role. `TransitionMeta` is the single constitutional metadata type.
...
Plus terminal states: `rejected`, `superseded`.
...
1. **Phase 0 (now):** Establish types and interfaces. No runtime changes.
   Phase 0 introduces constitutional typing for Tier A acts only; existing
   operational tables and handlers remain the runtime substrate unless
   explicitly wrapped.
...
## Type Skeleton Admission Rule

> Prose may anticipate. Types should ratify.

A type enters `packages/core/src/types/constitution.ts` only when Phase 1 code
will consume it — wrapping a mutation, recording a decision, or checking an
invariant. Types that anticipate future phases belong in this manifesto until
they have a first consumer.

---

*This document is the constitutional reference for swarm architecture
decisions. The companion TypeScript skeleton
(`packages/core/src/types/constitution.ts`) provides the Phase 1 type
definitions.*
