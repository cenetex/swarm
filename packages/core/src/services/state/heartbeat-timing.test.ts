...
describe('heartbeat-timing', () => {
  ...
  describe('getLastHeartbeat', () => {
    it('returns 0 when no record exists', async () => {
      const result = await getLastHeartbeat(mockDocClient, TABLE, 'avatar-1', 'moltbook');
      expect(result).toBe(0);
    });

    it('returns the stored timestamp for a specific platform', async () => {
      const key = makeKey('AVATAR#avatar-1', 'HEARTBEAT#moltbook');
      store.set(key, { pk: 'AVATAR#avatar-1', sk: 'HEARTBEAT#moltbook', timestamp: 1700000000000 });
    ...
    it('returns different timestamps for different platforms', async () => {
      store.set(makeKey('AVATAR#avatar-1', 'HEARTBEAT#moltbook'), {
        pk: 'AVATAR#avatar-1', sk: 'HEARTBEAT#moltbook', timestamp: 1000,
      });
      store.set(makeKey('AVATAR#avatar-1', 'HEARTBEAT#twitter'), {
        pk: 'AVATAR#avatar-1', sk: 'HEARTBEAT#twitter', timestamp: 2000,
      });

      expect(await getLastHeartbeat(mockDocClient, TABLE, 'avatar-1', 'moltbook')).toBe(1000);
      expect(await getLastHeartbeat(mockDocClient, TABLE, 'avatar-1', 'twitter')).toBe(2000);
    });
    ...
    it('returns different timestamps for different avatars', async () => {
      ...
      store.set(makeKey('AVATAR#avatar-2', 'HEARTBEAT#moltbook'), {
        pk: 'AVATAR#avatar-2', sk: 'HEARTBEAT#moltbook', timestamp: 3000,
      });

      expect(await getLastHeartbeat(mockDocClient, TABLE, 'avatar-1', 'moltbook')).toBe(1000);
      expect(await getLastHeartbeat(mockDocClient, TABLE, 'avatar-2', 'moltbook')).toBe(3000);
    });
  });

  describe('setLastHeartbeat', () => {
    it('writes correct DynamoDB item with platform in sort key', async () => {
      await setLastHeartbeat(mockDocClient, TABLE, 'avatar-1', 'moltbook', 1700000000000);
      ...
      expect(item!.sk).toBe('HEARTBEAT#moltbook');
      expect(item!.platform).toBe('moltbook');
    ...
    it('overwrites existing heartbeat for same avatar and platform', async () => {
      await setLastHeartbeat(mockDocClient, TABLE, 'avatar-1', 'moltbook', 1000);
      await setLastHeartbeat(mockDocClient, TABLE, 'avatar-1', 'moltbook', 2000);
