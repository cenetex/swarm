...
export async function executeFallbackToolLoop(params: {
  ...
  while (currentToolCalls.length > 0 && fallbackStep < MAX_FALLBACK_TOOL_STEPS) {
    ...
    if (nextPauseTool && mcpServices && avatarId) {
      let pauseToolName: string = String(nextPauseTool.name);

      // Detect looping: if the same pause tool is being called repeatedly, break the loop
      if (pauseToolName === lastPauseToolName && lastPauseToolName !== null) {
        ...
        if (pauseToolRepeatCount >= 2) {
          ...
          const toolDisplayName = (lastPauseToolName as string).replace(/_/g, ' ');
          response = `I encountered an issue while trying to access the ${toolDisplayName} interface. This might be due to a network error or misconfiguration. Please try refreshing the page or starting a new conversation.`;
