...
vi.mock('@swarm/core', () => ({
  ...
  // Transitive exports needed by handler dependency chain
  createSqsOffloadServiceFromEnv: () => null,
  createGallerySaver: () => ({ save: () => Promise.resolve() }),
  createRuntimeMetricsLogger: () => ({ log: () => {}, incrementCounter: () => {}, recordDuration: () => {}, trackDuration: () => {}, setProperty: () => {}, flush: () => Promise.resolve() }),
  createLLMService: () => ({}),
  createMessageEvaluator: () => ({ evaluate: () => ({ shouldRespond: true }) }),
  createIdentityLinkService: () => ({}),
  createContentStoreService: () => ({}),
  SwarmErrorCode: { LLM_MISSING_API_KEY: 'LLM_MISSING_API_KEY', LLM_CIRCUIT_OPEN: 'LLM_CIRCUIT_OPEN', LLM_API_ERROR: 'LLM_API_ERROR', LLM_EMPTY_RESPONSE: 'LLM_EMPTY_RESPONSE' },
  LLMError: class LLMError extends Error { code: string; constructor(msg: string, opts?: any) { super(msg); this.code = opts?.code ?? ''; } },
  AuthError: class AuthError extends Error { code: string; statusCode: number; constructor(msg: string, opts?: any) { super(msg); this.code = opts?.code ?? ''; this.statusCode = opts?.statusCode ?? 500; } },
  formatContinuationAsSystemMessage: () => '',
  isProgressUpdate: () => false,
  getHeaderValue: () => null,
  hasValidInternalTestKey: () => false,
  DEFAULT_LLM_MODEL: 'anthropic/claude-3-5-sonnet-latest',
  RATICROSS_PROTOCOL_VERSION: '1.0.0',
  shouldTriggerAvatarLoop: () => false,
  enqueuePost: () => {},
  enqueueMediaJob: () => {},
