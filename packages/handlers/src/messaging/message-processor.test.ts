...
describe('Discord Self-Reply Loop Protection', () => {
  describe('Own-message filter (issue #1523)', () => {
    it('should drop own messages by avatar (prevent self-reply)', () => {
      const { buildDiscordEnvelope } = require('./../../core/src/platforms/discord.js');
      const message = {
        id: '123',
        channel_id: 'ch-1',
    ...
    it('should accept messages from other users', () => {
      const { buildDiscordEnvelope } = require('./../../core/src/platforms/discord.js');
      const message = {
        id: '123',
        channel_id: 'ch-1',
        author: { id: 'user-111', username: 'user', bot: false },
        content: 'test',
        timestamp: new Date().toISOString(),
        mentions: [],
        attachments: [],
  ...
  describe('Bot message filter default (ignoreBots)', () => {
    it('should drop other bot messages by default to prevent bot ping-pong', () => {
      const { buildDiscordEnvelope } = require('./../../core/src/platforms/discord.js');
      const message = {
        id: '123',
        channel_id: 'ch-1',
        author: { id: 'other-bot-123', username: 'otherbot', bot: true },
        content: 'test',
        timestamp: new Date().toISOString(),
        mentions: [],
        attachments: [],
