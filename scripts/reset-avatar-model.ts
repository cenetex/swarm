...
async function main() {
  ...
  if (!avatarId) {
    console.error('Usage: node scripts/reset-avatar-model.ts <env> <avatarId> [model]');
    process.exit(1);
  }
  ...
  const hasLlmConfig = !!getResult.Item?.llmConfig;

  await client.send(
    new UpdateItemCommand({
      TableName: tableName,
      Key: key,
      UpdateExpression: hasLlmConfig
        ? 'SET llmConfig.model = :model'
        : 'SET llmConfig = :llmConfig',
      ExpressionAttributeValues: hasLlmConfig
        ? { ':model': { S: model } }
        : { ':llmConfig': { M: { model: { S: model } } } },
    }),
  );
